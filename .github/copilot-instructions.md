...
- When a TypeScript type-related error occurs, call the `able_annotation` tool with {"filePath":"<abs path>","code":"<small code fragment including the error and ~2 lines of context>"}; do not call it for pure syntax errors, missing-module errors, or lint-only issues. After the tool returns, inspect the annotated code for comments like "// <var> satisfies <Type>" and read the accompanying JSON for canonical types and definitions, noting that the JSON includes the absolute path to the type definition file for loading or reference.
