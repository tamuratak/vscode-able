...
function registerTerminalAndSideBarCommand() {
    return [
...
function registerFocusTerminalCommand() {
...
function registerRecenterCommand() {
    return [
        vscode.commands.registerCommand('able.recenter', async () => {
            ...
            if (editor && cursor) {
                ...
                highlightCursor(editor)
            }
        })
    ]
}

function highlightCursor(editor: vscode.TextEditor) {
    const cursor = editor.selection.active
...
function registerKillLinesToEndCommand() {
    return [
