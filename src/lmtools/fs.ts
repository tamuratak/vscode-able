...
/*
export class RepositoryTreeTool implements LanguageModelTool<string> {
    ...
    async invoke(options: LanguageModelToolInvocationOptions<string>) {
        ...
        const uri = await findWorkspaceFileUri(options.input)
        if (!uri) {
            const message = `RepositoryTreeTool uri is undefined: ${options.input}`
            this.extension.outputChannel.error(message)
            throw new Error(message)
        }
        const fileUint8Array = await vscode.workspace.fs.readDirectory(uri)
