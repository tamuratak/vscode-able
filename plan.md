...
- Playwright REPL のローカルアクセスをさらに限定し、接続可能ポートを 3000-3010 のみに固定する
- URL 判定を明確化し、曖昧な許可（ポート省略や about/data）を排除する

## 事前合意（ユーザー確認済み）

- 3000-3010 制限は localhost / 127.0.0.1 / ::1 のすべてに適用する
- URL にポート明記がない場合は拒否する
- 許可スキームは http / https のみとし、about: / data: は拒否する

## 変更方針

1. URL 許可判定を host 判定に加えて scheme と port の厳格判定へ変更する
2. 許可ポート範囲を 3000-3010 の inclusive 範囲に固定する
3. ユニットテストを新仕様に合わせて更新する
4. 既存統合テストのローカル HTTP サーバー起動ポートを許可範囲内に固定する

## 受け入れ条件

- `http://localhost:3000` は許可される
- `http://127.0.0.1:3010` は許可される
- `http://localhost:2999` と `http://localhost:3011` は拒否される
- `http://localhost`（ポート省略）は拒否される
- `about:blank` と `data:` URL は拒否される
- 変更後に `get_errors` で全体エラー 0
