...
import { scanHtmlTag2 } from '../../../src/chat/fixmathlib/utils.js'

suite('fixmath.scanHtmlTag2', () => {

    test('simple opening tag moves past closing bracket', () => {
        const txt = '<p>text'
        assert.strictEqual(scanHtmlTag2(txt, 0), txt.indexOf('>') + 1)
    })
    ...
    test('self-closing tag with attributes matches end', () => {
        ...
        assert.strictEqual(scanHtmlTag2(txt, 0), txt.indexOf('/>') + 2)
    })

    test('index not at tag returns 0', () => {
        const txt = 'prefix <b>bold</b>'
        assert.strictEqual(scanHtmlTag2(txt, 0), 0)
    })
